...
- (float)checkTmpSize {
    float totalSizeOne = 0;
    float totalSizeTwo = 0;

    NSString *str = [diskCachePath stringByReplacingOccurrencesOfString:@"ImageCache" withString:@"com.lanou3g.YouWoYou/fsCachedData"];
    NSLog(@"%@", str);
    NSDirectoryEnumerator *fileEnumerator = [[NSFileManager defaultManager] enumeratorAtPath:str];
    for (NSString *fileName in fileEnumerator) {
        NSString *filePath = [str stringByAppendingPathComponent:fileName];
        NSDictionary *attrs = [[NSFileManager defaultManager] attributesOfItemAtPath:filePath error:nil];
        unsigned long long length = [attrs fileSize];
        totalSizeOne += length / 1024.0 / 1024.0;
    }

    NSDirectoryEnumerator *fileEnumeratorTwo = [[NSFileManager defaultManager] enumeratorAtPath:diskCachePath];
    for (NSString *fileName in fileEnumeratorTwo) {
        NSString *filePath = [diskCachePath stringByAppendingPathComponent:fileName];
        NSDictionary *attrs = [[NSFileManager defaultManager] attributesOfItemAtPath:filePath error:nil];
        unsigned long long length = [attrs fileSize];
        totalSizeTwo += length / 1024.0 / 1024.0;
    }
    return totalSizeOne + totalSizeTwo;
